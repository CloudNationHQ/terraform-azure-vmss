  ...
  dynamic "os_profile" {
    ...
    content {
      ...
      dynamic "linux_configuration" {
        for_each = var.vmss.type == "flex" && (lower(try(var.vmss.os_type, "linux")) == "linux" || var.vmss.public_key != null || contains(keys(tls_private_key.tls_key), var.vmss.name) || try(var.vmss.generate_ssh_key.enable, false) == true) ? [1] : []
      ...
      dynamic "windows_configuration" {
        for_each = var.vmss.type == "flex" && lower(try(var.vmss.os_type, "linux")) == "windows" ? [1] : []
